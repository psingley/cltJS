...
/*
 * Bug(Collette Solution): event.stopImmediatePropagation() has to be called to prevent event bubbling / multiple calls (line 219)
 *
 */
...
(function($) {
    ...
    confirmOn.deleteOverlay = function($element) {
        var classPrepend = $element.data('confirmon').options.classPrepend;
        $('.' + classPrepend + '-overlay').fadeOut('fast', function(){
            $(this).remove();
        });
    };
    ...
    confirmOn.showBoxAndFocusNo = function($element) {
        ...
        $('.' + classPrepend + '-box').fadeIn(function(){
            ...
            $(this).children('button').eq(1).blur();
        });
    };

    confirmOn.deleteBox = function($element) {
        var classPrepend = $element.data('confirmon').options.classPrepend;
        $('.' + classPrepend + '-box').fadeOut('fast', function(){
            $(this).remove();
        });
    };
    ...
    confirmOn.convertArguments = function(options, events, selector, data, handler) {
        if (typeof options === 'object') {
            $.each(options, function(key, val) {
                if (typeof val === 'string' || key === 'onShow' || key === 'afterShow') { //Options provided so shift all args to left
    ...
    $.fn.confirmOn = function(options, events, selector, data, handler) {
        this.each(function(){
            ...
            function confirmHandler(event) {
                    ...
                    if (typeof $element.data('confirmon').options.afterShow === 'function') {
                        $element.data('confirmon').options.afterShow.call();
                    }
           };
        });
